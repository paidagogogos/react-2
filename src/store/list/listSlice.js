...
export const addList = createAsyncThunk(
  ...
  async(newList)=>{
    try{
        const res= await axios.post("http://localhost:3004/list", newList);
        return res.data;
    }catch(err){
        console.log(err)
    }
...
export const deleteList = createAsyncThunk(
  ...
  async(id)=>{
    try{
        const res= await axios.delete(`http://localhost:3004/list/${id}`);
        return id;
    }catch(err){
        console.log(err)
    }
...
export const updateList = createAsyncThunk(
  ...
  async({id,content})=>{
    try{
        const res= await axios.put(`http://localhost:3004/list/${id}`,{
          content:content
        });
        return {id,content}
    }catch(err){
